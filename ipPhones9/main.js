...
function renderActivityLog() {
  ...
  if (recentActivities.length === 0) {
    container.innerHTML = '<p class="text-muted text-sm">Нет записей</p>'
    return
  }
  ...
  container.innerHTML = recentActivities
    .map(
      (activity) => `
        <div class="flex items-start gap-3 p-3 glass rounded-lg border border-primary">
            <div class="flex-shrink-0 w-2 h-2 mt-2 rounded-full bg-blue-500"></div>
            <div class="flex-1 min-w-0">
                <p class="text-secondary text-sm">
                    <span class="font-bold text-primary">${activity.phoneModel}</span>
                    ${activity.action}
                    ${activity.details ? `<span class="text-muted">${activity.details}</span>` : ""}
                </p>
                <p class="text-muted text-xs mt-1">${activity.timestamp}</p>
            </div>
        </div>
    `,
    )
...
function applyTheme() {
  ...
  if (theme === "light") {
    ...
    cursorGlow.classList.add("hidden") // Hide cursor glow in light mode
  } else {
    themeName.textContent = "НЕОН ТЬМЫ"
    brandLogo.src = "/public/skuffcall-logo-neon.png"
    cursorGlow.classList.remove("hidden") // Show cursor glow in dark mode
  }
}
...
function renderTable(filteredPhones = phones) {
  ...
  headerRow.innerHTML =
    visibleColumns
      .map(
        ...
        <th 
            ...
            data-index="${index}"
            class="px-4 md:px-6 py-3 md:py-4 text-left font-bold uppercase tracking-wider cursor-move hover:bg-opacity-10 transition-colors group text-sm"
        >
            <div class="flex items-center justify-between gap-2">
                ...
                <button 
                    class="opacity-0 group-hover:opacity-60 hover:!opacity-100 transition-opacity text-yellow-600"
    ...
    <th class="px-4 md:px-6 py-3 md:py-4 text-center relative">
        ...
        <div id="hidden-columns-dropdown" class="hidden absolute right-0 top-full mt-2 glass border border-primary rounded-lg shadow-lg z-50 min-w-[200px]"></div>
    </th>
    `
    ...
    .map(
      (phone) => `
        <tr 
            ...
            class="table-row border-b cursor-pointer transition-all"
        >
            ${visibleColumns
              .map((col) => {
                if (col.id === "status") {
                ...
                return `<td class="px-4 md:px-6 py-3 md:py-4 table-cell text-sm">${phone[col.id] || "-"}</td>`
              })
              .join("")}
            <td></td>
        </tr>
  ...
  container.innerHTML = phonesToRender
    .map(
      (phone) => `
        <div 
            data-phone-id="${phone.id}"
            class="glass rounded-xl p-6 border border-primary hover-lift cursor-pointer fade-in"
        >
            <div class="flex items-start gap-4">
                <img src="${getPhoneImage(phone.model)}" alt="${phone.model}" class="w-20 h-20 object-contain rounded-lg border border-primary">
                <div class="flex-1">
                    <h3 class="text-lg font-bold text-primary mb-2">${phone.model}</h3>
            ...
            <div class="mt-4 space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="text-secondary font-mono">${phone.mac}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-muted">IP:</span>
                    <span class="text-secondary font-mono">${phone.ip || "-"}</span>
                </div>
                ${
                  phone.user
                    ? `
                <div class="flex justify-between">
                    ...
                    <span class="text-secondary">${phone.user}</span>
                </div>
                `
                    : ""
                }
                ...
                <div class="flex justify-between">
                    ...
                    <span class="text-secondary">${phone.department}</span>
                </div>
                `
                    : ""
                }
  ...
  if (dropdown.classList.contains("hidden")) {
    dropdown.innerHTML = hiddenColumns
      .map(
        (col) => `
        <button 
          class="w-full text-left px-4 py-3 text-secondary hover:bg-opacity-20 transition-colors border-b border-primary last:border-b-0"
...
function openEditModal(phoneId) {
  ...
  fieldsContainer.innerHTML = visibleColumns
    .map(
      (col) => `
        <div>
            <label class="block text-primary text-sm font-bold mb-2">${col.label}</label>
            ${
              col.id === "status"
                ? `
                <select name="${col.id}" class="w-full input-field border rounded-lg px-4 py-2 focus:outline-none">
            ...
            `
                : col.id === "notes"
                  ? `
                <textarea name="${col.id}" rows="3" class="w-full input-field border rounded-lg px-4 py-2 focus:outline-none resize-none">${phone[col.id] || ""}</textarea>
            `
                  : `
                <input type="text" name="${col.id}" value="${phone[col.id] || ""}" class="w-full input-field border rounded-lg px-4 py-2 focus:outline-none">
            `
            }
        </div>
    `,
...
function showNotification(message) {
  ...
  notification.className =
    "fixed bottom-4 right-4 glass border-2 border-yellow-500 text-yellow-600 px-6 py-4 rounded-lg z-50 fade-in"
  notification.innerHTML = `
    <div class="flex items-center gap-3">
      ...
      <button class="ml-4 hover:text-red-500" onclick="this.parentElement.parentElement.remove()">✕</button>
    </div>
  `
